...
def plot_graph(filenames, labels, output_image, missing_points):
    ...
    colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728']  # 默認顏色循環
...
def main():
    ...
    output_files = {
        "insertion_sort": "output_4_insertion_sort",
        "merge_sort": "output_4_merge_sort",
        "quick_hoare": "output_4_quick_hoare",
        "counting_sort": "output_4_counting_sort"
    }

    # 紀錄哪些資料點超時並需要預測
    missing_points = {
        "insertion_sort": {"k_value": [], "predicted_time": []},
        "merge_sort": {"k_value": [], "predicted_time": []},
        "quick_hoare": {"k_value": [], "predicted_time": []},
        "counting_sort": {"k_value": [], "predicted_time": []}
    }
    ...
    for k in range(0, 21):  # 可調整範圍
        ...
        for name, executable in cpp_programs.items():
            ...
            success = run_cpp_program(executable, size, "second", k_value, output_files[name], timeout=3000)
            if not success:
    ...
    # 繪製圖表
    plot_graph(list(output_files.values()), list(cpp_programs.keys()), "grapg_4.png", missing_points)
